...
window.ProjectPage = {
  ...
  render(container, projectId) {
    ...
    if (mode === "site") {
      ...
      const manageTabs = [
        ...
        { id: "documents", label: "Vault Documents",   icon: "shield",        roles: ['admin','designer','engineer','client','architect','site_engineer'] },
  ...
  renderTimeline(proj, role) {
    ...
    proj.updates.forEach((u, idx) => {
      events.push({
        ...
        content: u.completed,
        details: `Workers on-site: <b>${u.workersCount}</b>${u.issues !== 'None' ? `, Issues faced: <span class='text-rose-400 font-semibold'>${u.issues}</span>` : ""}, Target tomorrow: <i>${u.tomorrowPlan}</i>`,
        photo: u.photo,
  ...
  renderDocuments(proj, role) {
    ...
    // Render contracts, agreements, quotes and billing receipts
    const folders = [
      { id: "agreements", label: "Contracts & Agreements", icon: "file-signature" },
      { id: "quotations", label: "BOQ Quotations & Estimations", icon: "file-spreadsheet" },
      { id: "invoices", label: "Invoices & Payment Receipts", icon: "receipt" }
    ];
    ...
    return `
      <div class="card p-5 space-y-5">
        <div class="flex items-center justify-between border-b border-border-color pb-3">
          <div>
            <h3 class="text-sm font-bold text-white">Project Document Vault</h3>
            <p class="text-xs text-secondary mt-0.5">Secure repository for legal agreements, design quotations, and billing PDFs.</p>
          </div>
          ${isUploadingAllowed ? `
            <button onclick="openModal('add-document-modal')" class="px-2.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-black text-xs font-semibold rounded flex items-center gap-1">
        ...
        ${isUploadingAllowed ? `
          <div id="vault-drag-drop" class="border-2 border-dashed border-border-color/60 hover:border-amber-500 rounded-lg p-5 text-center cursor-pointer transition-colors relative bg-primary/20 hover:bg-hover/20">
            <input type="file" id="vault-file-uploader" class="absolute inset-0 opacity-0 cursor-pointer">
            <div class="flex flex-col items-center justify-center text-xs space-y-1.5">
              <i data-lucide="file-plus" class="w-7 h-7 text-amber-500"></i>
              <span class="font-semibold text-white">Drag & drop a file here, or click to browse</span>
              <span class="text-[10px] text-muted">Supports PDFs, images, text, and other documents</span>
            </div>
          </div>
        ` : ""}
        ...
        <div class="space-y-6">
          ${folders.map(fold => {
            const files = proj.vault[fold.id] || [];
            
            return `
              <div class="space-y-2">
                <div class="flex items-center gap-2 border-b border-border-color/30 pb-1">
                  <i data-lucide="${fold.icon}" class="w-4 h-4 text-amber-500"></i>
                  <h4 class="text-xs font-bold text-white uppercase tracking-wider font-mono">${fold.label}</h4>
                </div>
                <div class="space-y-1.5 pl-6">
                  ${files.map(f => `
                    <div class="flex items-center justify-between text-xs p-2 bg-primary/20 hover:bg-hover/30 rounded border border-border-color/10">
                      <div class="flex items-center gap-2 min-w-0">
                        <i data-lucide="file-text" class="w-4 h-4 text-muted flex-shrink-0"></i>
                        <span class="truncate text-white font-medium">${f.name}</span>
                        <span class="text-[9px] text-muted font-mono font-bold">(${f.size})</span>
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="text-[9px] text-muted font-mono">${window.Utils.formatDate(f.date)}</span>
                        ${isUploadingAllowed ? `
                          <button onclick="window.ProjectPage.editDocument('${proj.id}', '${fold.id}', '${f.name}')" class="p-1 bg-hover hover:bg-border-color border border-border-color/25 rounded text-white" title="Edit Document Details"><i data-lucide="edit-2" class="w-3.5 h-3.5 text-amber-500"></i></button>
                          <button onclick="window.ProjectPage.deleteDocument('${proj.id}', '${fold.id}', '${f.name}')" class="p-1 bg-hover hover:bg-border-color border border-rose-500/20 rounded text-white" title="Delete Document"><i data-lucide="trash-2" class="w-3.5 h-3.5 text-rose-500"></i></button>
                        ` : ""}
                        <button onclick="window.ProjectPage.viewVaultDocument('${proj.id}', '${fold.id}', '${f.name.replace(/'/g, "\\'")}')" class="p-1 bg-hover hover:bg-border-color rounded border border-border-color/25 text-white flex items-center gap-1 text-[9px] uppercase font-bold font-mono">
                          <i data-lucide="external-link" class="w-3.5 h-3.5"></i> Open
                        </button>
                      </div>
                    </div>
                  `).join("")}
                  ${files.length === 0 ? `<span class="text-[10px] text-muted italic block py-1">No documents uploaded under this category yet.</span>` : ""}
                </div>
              </div>
            `;
          }).join("")}
  ...
  editStage(projId, stageName) {
    ...
    if (!stage) return;
    window.openModal("edit-stage-modal", { stage });
  },
