  ...
  (async () => {
    try {
      ...
      if (profile) {
        ...
        const areArraysEqual = (arr1, arr2, idKey = 'id') => {
          if (!arr1 || !arr2) return false;
          if (arr1.length !== arr2.length) return false;
          const ids1 = arr1.map(x => String(x[idKey] || '')).sort().join(',');
          const ids2 = arr2.map(x => String(x[idKey] || '')).sort().join(',');
          return ids1 === ids2;
        };

        if (cloudProjects && !areArraysEqual(cloudProjects, window.AppStore.state.projects, 'id')) {
          window.AppStore.state.projects = cloudProjects;
          needsRefresh = true;
        }
        if (cloudVendors && !areArraysEqual(cloudVendors, window.AppStore.state.vendors, 'name')) {
          window.AppStore.state.vendors = cloudVendors;
          needsRefresh = true;
        }
        if (cloudRFQs && !areArraysEqual(cloudRFQs, window.AppStore.state.rfqs, 'id')) {
          window.AppStore.state.rfqs = cloudRFQs;
          needsRefresh = true;
        }
        if (cloudPOs && !areArraysEqual(cloudPOs, window.AppStore.state.purchaseOrders, 'id')) {
          window.AppStore.state.purchaseOrders = cloudPOs;
          needsRefresh = true;
        }
        if (cloudPayouts && !areArraysEqual(cloudPayouts, window.AppStore.state.payouts, 'id')) {
        ...
        if (needsRefresh) {
          ...
          window.AppRouter.refresh();
          console.log("Premio Living OS data synced and refreshed from Supabase.");
        }
      }
    } catch (err) {
